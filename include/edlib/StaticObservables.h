...
namespace EDLib {
  ...
  public:
    ...
    const static std::string _BNI_BNJ_;
    ...
    std::map<std::string, std::vector<precision>> calculate_static_observables(Hamiltonian& ham){
      ...
      std::map<std::string, std::vector<precision>> avg = {
        ...
        {_BNI_BNJ_, std::vector<precision>(ham.model().interacting_orbitals() * ham.model().interacting_orbitals(), 0.0)},
...
//          std::cout<<"Skipped by Boltzmann factor."<<std::endl;
  ...
  private:
    ...
    std::map<std::string, std::vector<precision>> calculate_static_observables_eigenvector(Hamiltonian& ham, const EigenPair<precision, sector>& pair){
      ...
      std::vector<precision> bnibnj(ham.model().interacting_orbitals() * ham.model().interacting_orbitals(), 0.0);
        ...
        for(int orb = 0; orb < ham.model().interacting_orbitals(); ++orb){
          ...
          for(int orb2 = 0; orb2 < ham.model().interacting_orbitals(); ++orb2){
            ...
            if(orb != orb2){
              ...
              bnibnj[ham.model().interacting_orbitals() * orb + orb2] += (1 - el_up) * (1 - el_down) * (1 - el_up2) * (1 - el_down2) * weight;
      ...
      MPI_Reduce(bnibnj.data(), result[_BNI_BNJ_].data(), bnibnj.size(), alps::mpi::detail::mpi_type<precision>(), MPI_SUM, 0, ham.comm());
  ...
  template<class Hamiltonian>
  const std::string StaticObservables<Hamiltonian>::_BNI_BNJ_ = "barN_i barN_j";
  template<class Hamiltonian>
